...
import { BABYLON_MODELS } from './BabylonAssetUrls';
...
/**
 * Imports a decorative mesh from models.babylonjs.com. Visual-only — no physics.
 */
export async function placeBabylonModel(
  ...
  const rootUrl = subFolder
    ? `${BABYLON_MODELS}${subFolder.replace(/^\/+|\/+$/g, '')}/`
    : BABYLON_MODELS;
